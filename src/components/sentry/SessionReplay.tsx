...
const EVENT_ICON: Record<RumEvent['type'], React.ReactNode> = {
  ...
  custom: <ArrowRight className="text-nself-text-muted h-3.5 w-3.5" />,
}

export function SessionReplay({ sessionId, events, className }: SessionReplayProps) {
  if (events.length === 0) {
    return (
      <div className={clsx('border-nself-border rounded-lg border p-4 text-center', className)}>
  ...
  return (
    <div className={clsx('space-y-1', className)}>
      <p className="text-nself-text-muted font-mono text-xs">Session {sessionId}</p>
        ...
        {events.map((event) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              <p className="text-nself-text truncate text-xs font-medium">{event.label}</p>
              {event.url !== undefined && (
                <p className="text-nself-text-muted truncate font-mono text-xs">{event.url}</p>
              )}
              {event.errorMessage !== undefined && (
                <p className="mt-0.5 truncate text-xs text-red-300">{event.errorMessage}</p>
              )}
            </div>
