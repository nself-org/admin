...
export function ReleaseCascadeWidget({
  ...
  const { data, error, isLoading } = useSWR<ReleaseStatusData>('/api/release-status', fetcher, {
    ...
    <div className={cn('rounded-xl border border-gray-800 bg-gray-900/50 p-5', className)}>
        ...
        <div>
          ...
          {data && (
            <p className="mt-0.5 text-xs text-gray-500">
              Latest: <span className="font-mono text-gray-300">v{data.latest}</span>
            </p>
          )}
        </div>
        {data && (
      ...
      {isLoading && (
        <div className="space-y-2">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="h-8 animate-pulse rounded-lg bg-gray-800/60" />
          ))}
        </div>
      )}
          ...
          {data.artifacts.map((artifact) => (
            ...
            >
              <div className="flex min-w-0 items-center gap-2.5">
                <span
                  className={cn('h-2 w-2 flex-shrink-0 rounded-full', STATUS_DOTS[artifact.status])}
                />
