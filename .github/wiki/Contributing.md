...
Admin ships a pre-commit hook at `.git/hooks/pre-commit`. It runs automatically on every `git commit`.

### requireAuth gate (S122-T04)

Every new API route file under `src/app/api/` must import `requireAuth`. The hook rejects a commit if a new or modified `route.ts` is missing this import.

**Pattern every protected route must follow:**
...
If you are adding a new public route that must not require auth, add its path pattern to the allowlist in `.git/hooks/pre-commit`.

### AI attribution gate

The hook also rejects commits that contain AI tool attribution (`Co-Authored-By: Claude`, `generated by Claude`, `Anthropic`) in file content or the commit message. See the [No AI Attribution rule](https://nself.org/docs/contributing#attribution) for context.
