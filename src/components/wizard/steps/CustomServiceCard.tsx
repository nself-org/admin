    ...
    <div className="rounded-lg border border-zinc-200 p-4 dark:border-zinc-700">
        ...
        <div className="flex items-center space-x-2">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (svc) svc.name = e.target.value.toLowerCase().replace(/[^a-z0-9_-]/g, '')
              setConfig({
                ...config,
          ...
          <label className="mb-1 block text-xs font-medium text-zinc-700 dark:text-zinc-300">
            ...
            <span className="group relative ml-1 inline-block">
              ...
              <div className="absolute bottom-full left-1/2 z-50 mb-2 hidden w-64 -translate-x-1/2 rounded-lg bg-zinc-900 p-2 text-xs text-white shadow-lg group-hover:block">
                ...
                Leave empty for no routing, or specify subdomain (e.g., &quot;api&quot; for api.
                {config.domain}) or full URL
              </div>
            </span>
          </label>
          <input
      ...
      {service.route && (
        <div className="mt-2 text-xs text-zinc-500 dark:text-zinc-400">
          ...
          {service.route.includes('.') ? service.route : `${service.route}.${config.domain}`}
        </div>
      )}
    </div>
  )
