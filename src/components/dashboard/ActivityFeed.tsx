...
import { CheckCircle, Info, Play, RotateCw, Square, Terminal, XCircle } from 'lucide-react'
...
export function ActivityFeed({ events, maxEvents = 10 }: ActivityFeedProps) {
  ...
  return (
    <div className="rounded-lg border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900/50">
      <h3 className="mb-3 text-sm font-semibold text-zinc-900 dark:text-white">Recent Activity</h3>
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                ...
                {/* User Attribution */}
                {event.user && (
                  <p className="mt-1 text-xs text-zinc-500 dark:text-zinc-500">by {event.user}</p>
                )}
              </div>
            </div>
          </div>
