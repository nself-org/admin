...
export const toast = {
  ...
  error: (message: string, options?) => sonnerToast.error(message, { duration: 7000, ...options }),
  // ...
}
```
...
test('view services and start/stop', async ({ page }) => {
  ...
  await expect(page.locator('[data-service="postgres"] [data-status]')).toHaveAttribute(
    'data-status',
    'stopped'
  )
